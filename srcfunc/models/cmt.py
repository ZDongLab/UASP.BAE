...
class CMT(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000, embed_dims=[46,92,184,368], stem_channel=16, fc_dim=512,
...
def _create_cmt_model(model_params, pretrained=False, distilled=False, **kwargs):
    #default_cfg = _cfg()
    default_num_classes = model_params['num_classes']
    #print(model_params['image_size'])
    default_img_size = model_params['image_size']
    default_in_chans = model_params['args'].in_chans
    ...
    model = CMT(img_size=img_size, in_chans=default_in_chans, num_classes=num_classes, representation_size=repr_size, **kwargs)
    #model.default_cfg = default_cfg

    if pretrained:
        load_pretrained(
            model, num_classes=num_classes, in_chans=default_in_chans,
            filter_fn=partial(checkpoint_filter_fn, model=model))
    return model


@register_model
def cmt_ti(model_params, pretrained=False, **kwargs):
    """ 
    CMT-Tiny
    """
    model_kwargs = dict(qkv_bias=True, **kwargs)
    model = _create_cmt_model(model_params, pretrained=pretrained, **model_kwargs)
    return model

@register_model
def cmt_xs(model_params, pretrained=False, **kwargs):
    """ 
...
@register_model
def cmt_s(model_params, pretrained=False, **kwargs):
    """ 
    CMT-Small
    """
    model_kwargs = dict(
...
@register_model
def cmt_b(model_params, pretrained=False, **kwargs):
    """ 
    CMT-Base
    """
    model_kwargs = dict(
