...
class Res2NeXt(nn.Module):
    def __init__(self, block, baseWidth, cardinality, layers, num_classes, scale=4, in_chans=3):
        """ Constructor
        Args:
...
def res2next28(model_params, pretrained=False, **kwargs):
    ...
    model = Res2NeXt(Bottle2neckX, layers = [3, 4, 6, 3], baseWidth = 4, cardinality=8, scale = 4, num_classes=num_classes, in_chans=in_chans)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['res2next50']))
    return model

def res2next50(model_params, pretrained=False, **kwargs):
    """    Construct Res2NeXt-50.
    The default scale is 4.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
...
def res2next101(model_params, pretrained=False, **kwargs):
    """    Construct Res2NeXt-50.
    The default scale is 4.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
