...
class Res2Net(nn.Module):
    ...
    def __init__(self, block, layers, baseWidth = 26, scale = 4, num_classes=1000, in_chans=3):
        self.inplanes = 64
        ...
        self.head = nn.Sequential(OrderedDict([('fc', self.fc)]))
...
def res2net50(model_params, pretrained=False, **kwargs):
    ...
    model = Res2Net(Bottle2neck, [3, 4, 6, 3], baseWidth = 26, scale = 4, num_classes=num_classes,in_chans=in_chans, **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['res2net50_26w_4s']))
    return model

def res2net50_26w_4s(model_params, pretrained=False, **kwargs):
...
def res2net101_26w_4s(model_params, pretrained=False, **kwargs):
    """Constructs a Res2Net-50_26w_4s model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    in_chans = model_params['args'].in_chans
    num_classes = model_params['num_classes']
    model = Res2Net(Bottle2neck, [3, 4, 23, 3], baseWidth = 26, scale = 4,num_classes=num_classes,in_chans=in_chans, **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['res2net101_26w_4s']))
    return model

def res2net50_26w_6s(model_params, pretrained=False, **kwargs):
    """Constructs a Res2Net-50_26w_4s model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    in_chans = model_params['args'].in_chans
    num_classes = model_params['num_classes']
    model = Res2Net(Bottle2neck, [3, 4, 6, 3], baseWidth = 26, scale = 6, num_classes=num_classes,in_chans=in_chans, **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['res2net50_26w_6s']))
    return model

def res2net50_26w_8s(model_params, pretrained=False, **kwargs):
    """Constructs a Res2Net-50_26w_4s model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    in_chans = model_params['args'].in_chans
    num_classes = model_params['num_classes']
    model = Res2Net(Bottle2neck, [3, 4, 6, 3], baseWidth = 26, scale = 8,num_classes=num_classes, in_chans=in_chans,  **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['res2net50_26w_8s']))
    return model

def res2net50_48w_2s(model_params, pretrained=False, **kwargs):
    ...
    in_chans = model_params['args'].in_chans
    num_classes = model_params['num_classes']
    model = Res2Net(Bottle2neck, [3, 4, 6, 3], baseWidth = 48, scale = 2, num_classes=num_classes, in_chans=in_chans, **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['res2net50_48w_2s']))
    return model

def res2net50_14w_8s(model_params, pretrained=False, **kwargs):
    ...
    in_chans = model_params['args'].in_chans
    num_classes = model_params['num_classes']
    model = Res2Net(Bottle2neck, [3, 4, 6, 3], baseWidth = 14, scale = 8, num_classes=num_classes, in_chans=in_chans, **kwargs)
    if pretrained:
